function [ele_MSE, var_in_theory] = dft_matrix_LS(N, num_tx_ant, num_rx_ant, K, T, corr_coef, no_var)
%     %% configurations
%     tx_power = 1;
%     N = 1000; % monte-carlo
% 
...
%     no_var = 0.01;
    ...
    for t = 1:T
        for k = 1:(K+1)
            PHI_dft(t,k) = exp(-1i*2*pi*(t-1)*(k-1)/T);
        end
    end

    % genrate observation matrix for DFT-matrix method
    H = X * (kron(PHI_dft, eye(num_rx_ant, num_rx_ant))); 

    % compute covariance matrix
    cov_mat = no_var * inv(H'*H);
    % cov_mat_2 = no_var * kron(inv(PHI_dft'*PHI_dft),eye(num_rx_ant,num_rx_ant));
    ...
    for iter = 1:N
        ...
        [theta, corr_mat] = generate_channel(num_rx_ant*(K + 1),num_tx_ant,corr_coef); % equivalent SIMO channels
